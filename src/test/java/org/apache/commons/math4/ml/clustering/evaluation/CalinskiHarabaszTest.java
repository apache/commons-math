...
public class CalinskiHarabaszTest {
    ...
    public void test_k_equals_4_is_best_for_a_4_center_points() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (score > expectBestScore) expectBestScore = score;
    ...
    public void test_compare_to_skLearn() {
        ...
        double k4EvaluateValue = 0.0;
        DistanceMeasure distanceMeasure = new EuclideanDistance();
        double maxEvaluateValue = 0.0;
        for (int i = 0; i < 5; i++) {
            final int k = i + 2;
            KMeansPlusPlusClusterer<DoublePoint> kMeans = new KMeansPlusPlusClusterer<>(k, -1, distanceMeasure, rnd);
            List<CentroidCluster<DoublePoint>> clusters = kMeans.cluster(points);
            double evaluateValue = evaluator.compute(clusters);
            if (evaluateValue > maxEvaluateValue) maxEvaluateValue = evaluateValue;
            if (k == kFromSkLearn) k4EvaluateValue = evaluateValue;
//            System.out.format("%d: %f\n", k, evaluateValue);
            // The score is approximately equals sklearn's score when k is smaller or equals to best k.
            if (k <= kFromSkLearn) Assert.assertEquals(scoreFromSkLearn[i], evaluateValue, 0.001);
        }
        // k=4 get the highest score
        Assert.assertEquals(maxEvaluateValue, k4EvaluateValue, 0.0);
    }
